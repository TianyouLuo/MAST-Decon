...
results = train.smooth(spatialRNA = spatialRNA, theta0 = w, UMI = UMI, 
                       ...
                       max_CT = 4, threshold = 0.02)

results_tibb = as_tibble(results, rownames = "barcodes")
write_csv(results_tibb, file.path(resultdir, paste0("smoothing_", smoothing, "C_max4.csv")))
...
ggplot() +
  ...
  coord_equal() +
  ggsave(file.path(resultdir, paste0("smoothing_",smoothing, "C_max4.png")), width = 12, height = 8)
